...
async function stripeRequest(key,params,idempotencyKey){const form=new URLSearchParams();for(const [k,v] of Object.entries(params))if(v!==undefined&&v!==null)form.append(k,String(v));const r=await fetch('https://api.stripe.com/v1/checkout/sessions',{method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/x-www-form-urlencoded','Idempotency-Key':idempotencyKey},body:form});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data?.error?.message||`Stripe checkout creation failed (${r.status})`);return data}
module.exports=async function handler(req,res){
 const {stripeKey,supabaseUrl,anonKey}=env();
 if(req.method==='GET')return send(res,200,{ok:true,service:'drivers-lounge-stripe-checkout',mode:process.env.VERCEL_ENV||'unknown',paymentsEnabled:process.env.COMMERCIAL_PAYMENTS_ENABLED==='true',stripeSecretConfigured:Boolean(stripeKey),supabaseAuthConfigured:Boolean(supabaseUrl&&anonKey)});
 if(req.method!=='POST'){res.setHeader('Allow','GET, POST');return send(res,405,{ok:false,error:'Method not allowed'})}
 if(process.env.COMMERCIAL_PAYMENTS_ENABLED!=='true')return send(res,503,{ok:false,error:'Online advertising payments are not open yet. Contact support to arrange a campaign.'});
 if(process.env.VERCEL_ENV!=='production'&&!/^sk_test_/.test(stripeKey))return send(res,503,{ok:false,error:'Preview checkout requires a Stripe test key'});
 ...
 const campaignId=String(input.campaign_id||'').trim();if(!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(campaignId))return send(res,400,{ok:false,error:'A valid campaign ID is required'});
 try{
  let user;try{user=await supabaseFetch(supabaseUrl,'/auth/v1/user',{token,anonKey})}catch(_){return send(res,401,{ok:false,error:'Your session is no longer valid. Sign in again.'})}if(!user?.id)return send(res,401,{ok:false,error:'Your session is no longer valid'});
  ...
  if(['paid','comped','refunded'].includes(campaign.billing_status))return send(res,409,{ok:false,error:'This campaign is already paid'});
  ...
  const origin=process.env.VERCEL_ENV==='production'?'https://drivers-lounge.vercel.app':'https://drivers-lounge-git-rc2-store-ready-steves-projects-c46af6ff.vercel.app';
  const idempotencyKey=crypto.createHash('sha256').update(JSON.stringify([campaign.id,user.id,amount,campaign.title,rate.name,Math.floor(Date.now()/1800000)])).digest('hex');
  const session=await stripeRequest(stripeKey,{
   ...
   'metadata[quoted_amount_cents]':amount,
   'payment_intent_data[metadata][campaign_id]':campaign.id,
   'payment_intent_data[metadata][submitted_by]':user.id,
   'payment_intent_data[metadata][quoted_amount_cents]':amount
  },idempotencyKey);
  return send(res,200,{ok:true,url:session.url,session_id:session.id,amount_cents:amount,campaign_id:campaign.id});
 }catch(e){console.error('Stripe checkout creation failed',{message:e.message,campaignId});return send(res,500,{ok:false,error:'Could not start checkout. Please retry or contact support.'})}
};
