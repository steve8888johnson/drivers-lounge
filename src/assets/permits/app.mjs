...
function changeGuard() {
  if (!dirty || window.confirm('Discard unsaved fields before switching?')) { dirty = false; return true; }
  return false;
}
...
function readLoad() {
  const f = new FormData($('#load-form')); trip.name = String(f.get('name') || '').trim(); trip.departure = String(f.get('departure') || ''); trip.acknowledged = f.has('acknowledged');
  for (const key of Object.keys(trip.profile)) trip.profile[key] = String(f.get(key) || '').trim();
}
function renderList() {
  ...
  document.querySelectorAll('[data-permit]').forEach(b => b.onclick = () => { if (!changeGuard()) return; selected = b.dataset.permit; renderReview(); renderList(); switchTab('review'); });
}
...
function readPermit() {
  const p = currentPermit(); if (!p) throw Error('Add a state permit first.'); const f = new FormData($('#permit-form'));
...
async function handleQR(raw) {
  const p = await ensurePermit(); readPermit(); edit(); const result = classifyQR(raw);
...
function displayFix(fix) {
  ...
  if (result.status !== 'on-route') { if (result.status !== 'gps-uncertain') stopGuidance(result.message); navAlert(result.message, result.status); $('#maneuver').textContent = 'Guidance paused — verify route'; $('#next-maneuver').textContent = 'Follow the issued permit. No automatic reroute.'; $('#maneuver-distance').textContent = '—'; $('#remaining').textContent = '—'; return; }
...
async function startGuidance() {
  ...
  timer = setInterval(() => { if (!lastFix || Date.now() - lastFix.timestamp > 15000) { navAlert('GPS is stale. Guidance paused; follow the issued permit.', 'stale'); $('#maneuver').textContent = 'Waiting for reliable GPS'; $('#maneuver-distance').textContent = '—'; } }, 5000);
...
on('#new-trip', 'click', async () => { if (changeGuard()) await openTrip(blankTrip()); });
on('#trip-picker', 'change', async e => { if (!changeGuard()) { e.target.value = trip.id; return; } const all = await listTrips(); await openTrip(all.find(t => t.id === e.target.value)); });
on('#examples', 'click', async () => { if (!changeGuard()) return; await openTrip(exampleTrip()); status('Photo-reference example loaded. Originals, Ohio route remainder, Indiana permit, load dimensions and geometry must be supplied.'); switchTab('route'); });
on('#add-permit', 'click', async () => { if (!changeGuard()) return; edit(); const p = blankPermit(); trip.permits.push(p); selected = p.id; await persist(); renderList(); renderReview(); switchTab('review'); });
on('#remove-permit', 'click', async () => { const p = currentPermit(); if (!p || !window.confirm(`Remove ${p.state || 'this'} permit from this load?`)) return; edit(); trip.permits = trip.permits.filter(x => x.id !== p.id); selected = trip.permits[0]?.id; await persist(); renderList(); renderReview(); });
on('#delete-trip', 'click', async () => { if (!window.confirm('Delete this device copy and its original documents? Export a backup first. This does not close a shared crew trip.')) return; stopGuidance(); await deleteTrip(trip); const all = await listTrips(); await openTrip(all[0] || blankTrip()); });
...
on('#import-packet', 'change', async e => { const file = e.target.files[0]; e.target.value = ''; if (!file) return; if (!changeGuard()) return; if (file.size > 65000000) throw Error('Trip packet exceeds 65 MB.'); const imported = await importPacket(JSON.parse(await file.text())); await openTrip(imported); status('Trip and originals imported. Confirm each permit and geometry on this device.'); });
on('#crew-role', 'change', async () => { if (watching !== null) stopGuidance('Role changed. Restart guidance to use the selected role.'); trip.localRole = $('#crew-role').value; await persist(); });
...
on('#close-crew', 'click', async () => { if (!trip.shared?.owner) throw Error('Only the trip owner can close live sharing.'); stopGuidance(); await crew.close(trip.shared); delete trip.shared; await persist(); $('#crew-status').textContent = 'Live trip closed. Crew access to fresh positions is disabled.'; });
