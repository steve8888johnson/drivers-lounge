const CACHE='drivers-lounge-rc2-permit-v7';
...
const SHELL=['/offline','/manifest.webmanifest','/assets/drivers-lounge-logo.png','/assets/rc1-design-system.css',...PERMIT_SHELL,...REFERENCE_SHELL];
...
self.addEventListener('message',event=>{
 if(event.data?.type!=='reference-cache-status'||!event.ports[0])return;
 event.waitUntil(caches.open(CACHE).then(async cache=>{
  const entries=await Promise.all(REFERENCE_SHELL.map(path=>cache.match(path)));
  event.ports[0].postMessage({referenceReady:entries.every(Boolean)});
 }).catch(()=>event.ports[0].postMessage({referenceReady:false})));
});
self.addEventListener('fetch',event=>{
 ...
 if(event.request.mode==='navigate'){
  const page=['/compliance','/compliance.html'].includes(url.pathname)?'/compliance':(['/permitted-loads','/permitted-loads.html'].includes(url.pathname)&&!url.search?'/permitted-loads':null);
  if(page){
