...
const launch=await read('src/launch.html'); if(!/018_rc2_live_schema_reconciliation\.sql/.test(launch)||!/backend unavailable/i.test(launch))fail('Launch checklist must describe the complete RC2 migration sequence and no-backend behavior'); else pass('Launch checklist reflects the current RC2 database and runtime gates');
...
if(pkg.scripts?.['check:supabase-contract']!=='node scripts/check-supabase-contract.mjs')fail('package.json must expose the Supabase schema-contract gate');
...
if(!/requiredSupabaseTables/.test(preflight))fail('Supabase preflight must use the canonical schema contract');
