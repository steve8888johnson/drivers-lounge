...
create policy "community rooms owner update" on community_rooms
for update to authenticated using (auth.uid() = creator_user_id) with check (auth.uid() = creator_user_id);

drop policy if exists "community rooms owner delete" on community_rooms;
create policy "community rooms owner delete" on community_rooms
for delete to authenticated using (auth.uid() = creator_user_id);

drop policy if exists "community posts author update" on community_posts;
create policy "community posts author update" on community_posts
...
create policy "community posts author delete" on community_posts
...
create policy "road reports author update" on road_reports
for update to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);

drop policy if exists "road reports author delete" on road_reports;
create policy "road reports author delete" on road_reports
...
create policy "carrier business response insert" on carrier_responses
for insert to authenticated with check (
  exists (
    select 1 from business_accounts b
    where b.id = business_account_id and b.owner_user_id = auth.uid()
  )
);
...
create policy "reporter reads own review reports" on carrier_review_reports
