...
-- 005_rc1_platform_groundwork.sql defines road_reports.user_id/report_type and no status column.
drop policy if exists "road reports author update" on road_reports;
create policy "road reports author update" on road_reports for update to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "road reports author delete" on road_reports;
create policy "road reports author delete" on road_reports for delete to authenticated using (auth.uid() = user_id);
